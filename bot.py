...
- Поддержка тем (топиков) в супергруппах
...
class Database:
    ...
    @staticmethod
    async def add_mailing(account_id: int, chats: List[str], topics: Dict[str, int] = None,
                         delay: int = 30, cycles: int = 10, message_html: str = "",
                         notifications_enabled: bool = True) -> int:
        async with db_pool.acquire() as conn:
            row = await conn.fetchrow(
                "INSERT INTO mailings (account_id, chats, topics, delay, cycles, message_html, status, notifications_enabled) "
                "VALUES ($1, $2, $3, $4, $5, $6, 'running', $7) RETURNING id",
                account_id, ",".join(chats), json.dumps(topics or {}), delay, cycles, message_html, notifications_enabled
            )
            return row['id']
...
async def verify_session_string(session_string: str) -> Optional[Dict]:
    ...
    try:
        ...
        try:
            ...
            logger.error(f"RPC ошибка при проверке сессии: {e}")
            return None
        except Exception as e:
...
async def get_forum_topics(client: TelegramClient, chat_id: int) -> List[Dict]:
    try:
        ...
        logger.info(f"Получено {len(topics)} тем из форума {chat_id}")
        return topics
    except Exception as e:
        logger.error(f"Ошибка получения тем форума: {e}")
        return []

async def send_message_to_chat(client: TelegramClient, chat_id: str, message: str, topic_id: int = None):
    try:
    ...
    try:
        ...
        for msg in messages:
            if msg.message:
                ...
                if 'навсегда' in text or 'never' in text or 'permanently' in text:
                ...
                elif numbers and any(word in text for word in ['до', 'until', 'ограничен', 'limited']):
                    return {
                        "blocked": True,
                        "permanent": False,
                    ...
                    return {
                        "blocked": False,
                        "message": "Спам-блок отсутствует"
                    }
        
        return {
            "blocked": False,
            "message": "Спам-блок не обнаружен"
        }
...
async def process_code(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            f'<b><tg-emoji emoji-id="{EMOJI["cross"]}">❌</tg-emoji> Код истек!</b>',
            parse_mode=ParseMode.HTML,
            reply_markup=get_account_manager_keyboard()
        )
        await cleanup_auth_client(user_id)
        ...
        await message.answer(
            f'<b><tg-emoji emoji-id="{EMOJI["cross"]}">❌</tg-emoji> Неверный код!</b>',
            parse_mode=ParseMode.HTML,
            reply_markup=get_back_keyboard()
        )
    except Exception as e:
...
async def show_chats_page(message: Message, state: FSMContext):
    ...
    text = (
        ...
        '<i>Чаты с пометкой [ФОРУМ] имеют темы - нужно будет выбрать тему</i>'
    )
...
async def confirm_chat_selection(callback: CallbackQuery, state: FSMContext):
    ...
    forums = []
    for dialog in dialogs:
        if dialog['id'] in selected_chats and dialog.get('is_forum'):
            forums.append(dialog)
    
    if forums:
        await state.update_data(forums_to_process=forums, forum_topics={})
        await callback.message.delete()
        await callback.message.answer(
            f'<b><tg-emoji emoji-id="{EMOJI["topic"]}">📝</tg-emoji> Выбранные чаты содержат форумы!</b>\n'
            f'Необходимо выбрать темы для каждого форума.\n\n'
            f'Начинаю загрузку тем...',
            parse_mode=ParseMode.HTML
        )
        
        await state.set_state(MailingSetup.selecting_topics)
        await show_forum_topics(callback.message, state, forums[0], 0)
    else:
        await callback.message.delete()
        await callback.message.answer(
...
async def show_forum_topics(message: Message, state: FSMContext, forum: Dict, forum_index: int):
    data = await state.get_data()
    account_id = data['account_id']
    forums = data.get('forums_to_process', [])
    ...
    await message.answer(
        f'<b><tg-emoji emoji-id="{EMOJI["loading"]}">🔄</tg-emoji> Загружаю темы для: {forum["name"]}...</b>',
        parse_mode=ParseMode.HTML
    )
    
    topics = await get_forum_topics(client, int(forum['id']))
    
    if not topics:
        await message.answer(
            f'<b><tg-emoji emoji-id="{EMOJI["cross"]}">❌</tg-emoji> Не удалось получить темы для {forum["name"]}</b>\n'
            'Пропускаем этот форум...',
            parse_mode=ParseMode.HTML
        )
        
        next_index = forum_index + 1
        if next_index < len(forums):
            await show_forum_topics(message, state, forums[next_index], next_index)
        else:
            await finish_topic_selection(message, state)
        return
    
    builder = InlineKeyboardBuilder()
    
    for topic in topics:
        topic_name = topic['title'][:40] + "..." if len(topic['title']) > 40 else topic['title']
        is_selected = str(forum['id']) in forum_topics and forum_topics[str(forum['id'])] == topic['id']
        prefix = "✅ " if is_selected else "⬜ "
        
        builder.row(InlineKeyboardButton(
            text=f"{prefix}{topic_name}",
            callback_data=f"select_topic_{forum['id']}_{topic['id']}"
        ))
    
    builder.row(InlineKeyboardButton(
        text="Пропустить (отправлять без темы)",
        callback_data=f"skip_forum_{forum['id']}"
    ))
    
    await message.answer(
        f'<b><tg-emoji emoji-id="{EMOJI["topic"]}">📝</tg-emoji> Выберите тему для:</b>\n'
        f'{forum["name"]}\n\n'
        f'Форум {forum_index + 1} из {len(forums)}',
        parse_mode=ParseMode.HTML,
        reply_markup=builder.as_markup()
    )

@dp.callback_query(StateFilter(MailingSetup.selecting_topics), F.data.startswith("select_topic_"))
async def select_forum_topic(callback: CallbackQuery, state: FSMContext):
    parts = callback.data.split("_")
    forum_id = parts[2]
    ...
    await state.update_data(forum_topics=forum_topics)
    
    forums = data.get('forums_to_process', [])
    current_index = next(i for i, f in enumerate(forums) if f['id'] == forum_id)
    next_index = current_index + 1
    
    await callback.message.delete()
    
    if next_index < len(forums):
        await show_forum_topics(callback.message, state, forums[next_index], next_index)
    else:
        await finish_topic_selection(callback.message, state)
    
    await callback.answer()

@dp.callback_query(StateFilter(MailingSetup.selecting_topics), F.data.startswith("skip_forum_"))
async def skip_forum_topic(callback: CallbackQuery, state: FSMContext):
    forum_id = callback.data.split("_")[2]
...
async def finish_topic_selection(message: Message, state: FSMContext):
    data = await state.get_data()
    selected_chats = data['selected_chats']
    
    await message.answer(
        f'<b><tg-emoji emoji-id="{EMOJI["check"]}">✅</tg-emoji> Темы выбраны!</b>\n'
    ...
    try:
        for cycle in range(1, cycles + 1):
            ...
            for chat_id_to_send in chats:
                ...
                try:
                    topic_id = topics.get(chat_id_to_send)
...
if __name__ == "__main__":
    ...
    print("• Поддержка тем в супергруппах")
