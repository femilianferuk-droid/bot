...
async def cmd_start(message: Message):
    ...
    await message.answer(
        f'<tg-emoji emoji-id="{EMOJI["bot"]}">🤖</tg-emoji> Добро пожаловать в бот для массовых рассылок!\n'
...
async def go_back(message: Message, state: FSMContext):
    ...
    if current_state:
        await state.clear()
    await message.answer(
        f'<tg-emoji emoji-id="{EMOJI["home"]}">🏘</tg-emoji> Главное меню:',
        parse_mode=ParseMode.HTML,
        reply_markup=get_main_keyboard()
    )
...
async def account_manager(message: Message):
    """Менеджер аккаунтов"""
    await message.answer(
        f'<tg-emoji emoji-id="{EMOJI["profile"]}">👤</tg-emoji> Менеджер аккаунтов\n'
        'Выберите действие:',
        parse_mode=ParseMode.HTML,
...
async def functions(message: Message):
    ...
    await message.answer(
        f'<tg-emoji emoji-id="{EMOJI["settings"]}">⚙</tg-emoji> Раздел в разработке',
        parse_mode=ParseMode.HTML,
        reply_markup=get_main_keyboard()
    )
...
async def mailing_menu(message: Message):
    ...
    if not active_account:
        await message.answer(
            f'<tg-emoji emoji-id="{EMOJI["cross"]}">❌</tg-emoji> Нет активного аккаунта! '
    ...
    await message.answer(
        f'<tg-emoji emoji-id="{EMOJI["send"]}">📨</tg-emoji> Меню рассылки\n'
...
async def add_account_start(message: Message, state: FSMContext):
    ...
    await message.answer(
        f'<tg-emoji emoji-id="{EMOJI["profile"]}">👤</tg-emoji> Введите номер телефона в международном формате\n'
...
async def process_phone(message: Message, state: FSMContext):
    ...
    if not phone.startswith('+') or len(phone) < 10:
        await message.answer(
            f'<tg-emoji emoji-id="{EMOJI["cross"]}">❌</tg-emoji> Неверный формат номера. '
    ...
    try:
        ...
        try:
            ...
            await message.answer(
                f'<tg-emoji emoji-id="{EMOJI["send"]}">📩</tg-emoji> Код подтверждения отправлен. '
        ...
        await message.answer(
            f'<tg-emoji emoji-id="{EMOJI["cross"]}">❌</tg-emoji> Ошибка при отправке кода: {str(e)}',
            parse_mode=ParseMode.HTML,
            reply_markup=get_main_keyboard()
        )
        await state.clear()
...
async def process_code(message: Message, state: FSMContext):
    ...
    # Сначала отключаем старый клиент если есть
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                await message.answer(
                    f'<tg-emoji emoji-id="{EMOJI["lock_closed"]}">🔒</tg-emoji> Аккаунт защищен двухфакторной аутентификацией.\n'
                    'Введите пароль 2FA:',
                    parse_mode=ParseMode.HTML,
                    reply_markup=get_back_keyboard()
                )
            ...
            if not has_2fa:
                ...
                await message.answer(
                    f'<tg-emoji emoji-id="{EMOJI["check"]}">✅</tg-emoji> Аккаунт {data["phone"]} успешно добавлен!',
                    parse_mode=ParseMode.HTML,
                    reply_markup=get_account_manager_keyboard()
                )
                await state.clear()
            ...
            await message.answer(
                f'<tg-emoji emoji-id="{EMOJI["cross"]}">❌</tg-emoji> Код подтверждения истек. '
            ...
            await message.answer(
                f'<tg-emoji emoji-id="{EMOJI["cross"]}">❌</tg-emoji> Неверный код подтверждения. Попробуйте снова.',
                parse_mode=ParseMode.HTML,
                reply_markup=get_back_keyboard()
            )
        except errors.FloodWaitError as e:
            await message.answer(
                f'<tg-emoji emoji-id="{EMOJI["clock"]}">⏰</tg-emoji> Слишком много попыток. '
        ...
        await message.answer(
            f'<tg-emoji emoji-id="{EMOJI["cross"]}">❌</tg-emoji> Ошибка при входе: {str(e)}',
            parse_mode=ParseMode.HTML,
            reply_markup=get_main_keyboard()
        )
        await state.clear()
...
async def process_2fa(message: Message, state: FSMContext):
    ...
    try:
        ...
        try:
            ...
            await message.answer(
                f'<tg-emoji emoji-id="{EMOJI["check"]}">✅</tg-emoji> Аккаунт {data["phone"]} успешно добавлен с 2FA!',
                parse_mode=ParseMode.HTML,
                reply_markup=get_account_manager_keyboard()
            )
            await state.clear()
            ...
            await message.answer(
                f'<tg-emoji emoji-id="{EMOJI["cross"]}">❌</tg-emoji> Неверный пароль 2FA. Попробуйте снова.',
                parse_mode=ParseMode.HTML,
                reply_markup=get_back_keyboard()
            )
        finally:
...
async def list_accounts(message: Message):
    ...
    if not accounts:
        await message.answer(
            f'<tg-emoji emoji-id="{EMOJI["info"]}">ℹ</tg-emoji> Нет сохраненных аккаунтов',
            parse_mode=ParseMode.HTML,
            reply_markup=get_account_manager_keyboard()
        )
        return
    ...
    await message.answer(
        f'<tg-emoji emoji-id="{EMOJI["list"]}">👥</tg-emoji> Список аккаунтов:\n'
...
async def select_account(callback: CallbackQuery):
    ...
    await callback.message.answer(
        f'<tg-emoji emoji-id="{EMOJI["check"]}">✅</tg-emoji> Выбран аккаунт: {account["phone"]}',
        parse_mode=ParseMode.HTML,
        reply_markup=get_account_manager_keyboard()
    )
...
async def start_mailing(message: Message, state: FSMContext):
    ...
    if not active_account:
        await message.answer(
            f'<tg-emoji emoji-id="{EMOJI["cross"]}">❌</tg-emoji> Сначала выберите активный аккаунт в менеджере!',
            parse_mode=ParseMode.HTML,
            reply_markup=get_main_keyboard()
        )
        return
    ...
    try:
        ...
        if not await telethon_client.is_user_authorized():
            await message.answer(
                f'<tg-emoji emoji-id="{EMOJI["cross"]}">❌</tg-emoji> Ошибка авторизации аккаунта. '
        ...
        await message.answer(
            f'<tg-emoji emoji-id="{EMOJI["download"]}">📥</tg-emoji> Загружаем список чатов...',
            parse_mode=ParseMode.HTML
        )
        ...
        if not dialogs:
            await message.answer(
                f'<tg-emoji emoji-id="{EMOJI["cross"]}">❌</tg-emoji> Не удалось получить список чатов',
                parse_mode=ParseMode.HTML,
                reply_markup=get_mailing_keyboard()
            )
            return
        ...
        await message.answer(
            f'<tg-emoji emoji-id="{EMOJI["cross"]}">❌</tg-emoji> Ошибка: {str(e)}',
            parse_mode=ParseMode.HTML,
            reply_markup=get_mailing_keyboard()
        )
...
async def show_chats_page(message: Message, state: FSMContext):
    ...
    text = (
        f'<tg-emoji emoji-id="{EMOJI["list"]}">👥</tg-emoji> Выберите чаты для рассылки:\n'
...
async def toggle_chat_selection(callback: CallbackQuery, state: FSMContext):
    ...
    if chat_id in selected_chats:
        ...
        if len(selected_chats) < 50:
            ...
            await callback.answer(
                f'<tg-emoji emoji-id="{EMOJI["cross"]}">❌</tg-emoji> Максимум 50 чатов!',
                show_alert=True
            )
            return
...
async def confirm_chat_selection(callback: CallbackQuery, state: FSMContext):
    ...
    if not selected_chats:
        await callback.answer(
            f'<tg-emoji emoji-id="{EMOJI["cross"]}">❌</tg-emoji> Выберите хотя бы один чат!',
            show_alert=True
        )
        return
    ...
    await callback.message.delete()
    await callback.message.answer(
        f'<tg-emoji emoji-id="{EMOJI["check"]}">✅</tg-emoji> Выбрано чатов: {len(selected_chats)}\n'
...
async def cancel_mailing_setup(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.delete()
    await callback.message.answer(
        f'<tg-emoji emoji-id="{EMOJI["cross"]}">❌</tg-emoji> Рассылка отменена',
        parse_mode=ParseMode.HTML,
        reply_markup=get_mailing_keyboard()
    )
    await state.clear()
...
async def process_delay(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            f'<tg-emoji emoji-id="{EMOJI["cross"]}">❌</tg-emoji> Введите целое число (секунды)',
            parse_mode=ParseMode.HTML,
            reply_markup=get_back_keyboard()
        )
        return
...
async def process_cycles(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            f'<tg-emoji emoji-id="{EMOJI["cross"]}">❌</tg-emoji> Введите целое положительное число',
            parse_mode=ParseMode.HTML,
            reply_markup=get_back_keyboard()
        )
        return
    ...
    await message.answer(
        f'<tg-emoji emoji-id="{EMOJI["write"]}">📝</tg-emoji> Введите текст сообщения в формате HTML:\n'
...
async def process_message_text(message: Message, state: FSMContext):
    ...
    if not message_text:
        await message.answer(
            f'<tg-emoji emoji-id="{EMOJI["cross"]}">❌</tg-emoji> Сообщение не может быть пустым',
            parse_mode=ParseMode.HTML,
            reply_markup=get_back_keyboard()
        )
        return
    ...
    confirmation_text = (
        f'<tg-emoji emoji-id="{EMOJI["info"]}">📋</tg-emoji> Подтвердите рассылку:\n'
...
async def start_mailing_process(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        f'<tg-emoji emoji-id="{EMOJI["rocket"]}">🚀</tg-emoji> Рассылка #{mailing_id} запущена!\n'
        f'Чатов: {len(data["selected_chats"])}\n'
        f'Циклов: {data["cycles"]}\n'
        f'Задержка: {data["delay"]} сек',
        parse_mode=ParseMode.HTML,
...
async def execute_mailing(
    ...
    try:
        for cycle in range(1, cycles + 1):
            ...
            if mailing_id not in active_mailing_tasks:
                await bot.send_message(
                    chat_id,
                    f'<tg-emoji emoji-id="{EMOJI["stop"]}">🛑</tg-emoji> Рассылка #{mailing_id} остановлена пользователем',
                    parse_mode=ParseMode.HTML
                )
            ...
            await bot.send_message(
                chat_id,
                f'<tg-emoji emoji-id="{EMOJI["send"]}">📤</tg-emoji> Рассылка #{mailing_id}: '
            ...
            await bot.send_message(
                chat_id,
                f'<tg-emoji emoji-id="{EMOJI["stats"]}">📊</tg-emoji> Цикл {cycle} завершен:\n'
            ...
            if cycle < cycles and mailing_id in active_mailing_tasks:
                await bot.send_message(
                    chat_id,
                    f'<tg-emoji emoji-id="{EMOJI["clock"]}">⏳</tg-emoji> Ожидание {delay} секунд до следующего цикла...',
                    parse_mode=ParseMode.HTML
                )
        ...
        if mailing_id in active_mailing_tasks:
            await bot.send_message(
                chat_id,
                f'<tg-emoji emoji-id="{EMOJI["check"]}">✅</tg-emoji> Рассылка #{mailing_id} успешно завершена!\n'
        ...
        await bot.send_message(
            chat_id,
            f'<tg-emoji emoji-id="{EMOJI["cross"]}">❌</tg-emoji> Ошибка в рассылке #{mailing_id}: {str(e)}',
            parse_mode=ParseMode.HTML
        )
...
async def stop_mailing(message: Message):
    ...
    if not active_mailing_tasks:
        await message.answer(
            f'<tg-emoji emoji-id="{EMOJI["info"]}">ℹ</tg-emoji> Нет активных рассылок для остановки',
            parse_mode=ParseMode.HTML,
            reply_markup=get_main_keyboard()
        )
        return
...
async def confirm_stop_mailing(callback: CallbackQuery):
    ...
    if mailing_id in active_mailing_tasks:
        ...
        await callback.message.delete()
        await callback.message.answer(
            f'<tg-emoji emoji-id="{EMOJI["stop"]}">🛑</tg-emoji> Рассылка #{mailing_id} остановлена',
            parse_mode=ParseMode.HTML,
            reply_markup=get_main_keyboard()
        )
    else:
        await callback.answer(
            f'<tg-emoji emoji-id="{EMOJI["cross"]}">❌</tg-emoji> Рассылка уже завершена',
            show_alert=True
        )
...
async def show_mailing_status(message: Message):
    ...
    if not active_mailing_tasks:
        await message.answer(
            f'<tg-emoji emoji-id="{EMOJI["info"]}">ℹ</tg-emoji> Нет активных рассылок',
            parse_mode=ParseMode.HTML,
            reply_markup=get_mailing_keyboard()
        )
        return
    
    status_text = f'<tg-emoji emoji-id="{EMOJI["stats"]}">📊</tg-emoji> Статус активных рассылок:\n\n'
