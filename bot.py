...
async def start_auto_reply(account_id: int, account_phone: str, session_string: str, reply_text: str, notifications_chat_id: int = None):
    ...
    # Останавливаем существующий если есть
    ...
    async def auto_reply_handler(event):
        try:
            # Проверяем, что сообщение из личного чата (не группа, не канал)
            if event.is_private:
                sender = await event.get_sender()
                
                # Не отвечаем ботам и сами себе
                ...
                # Уведомляем пользователя бота если нужно
...
async def delete_account(callback: CallbackQuery):
    ...
    # Останавливаем автоответчик если есть
    await stop_auto_reply(account_id)
...
# ========== Система рассылок (без изменений, уже была выше) ==========
# ... (весь код рассылок остается тем же)
...
async def main():
    try:
        ...
        # Останавливаем все автоответчики
