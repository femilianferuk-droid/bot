...
async def read_session_file(file_path: str) -> Optional[str]:
    ...
    # Пробуем читать как текст (StringSession)
    try:
        ...
        logger.info("Файл не является UTF-8 текстом, пробуем другие кодировки")
    except Exception as e:
        logger.warning(f"Ошибка чтения как текст: {e}")
    
    # Пробуем читать как бинарный файл .session
    try:
        import base64
        with open(file_path, 'rb') as f:
            ...
            # Проверяем, является ли это файлом SQLite (формат .session)
            if content[:16] == b'SQLite format 3\x00':
                logger.info("Обнаружен SQLite формат (стандартный .session файл Telethon)")
                
                # Конвертируем в base64 для хранения
                encoded = base64.b64encode(content).decode('ascii')
                
                # Пробуем создать StringSession из закодированных данных
                try:
                    ...
                    if await test_client.is_user_authorized():
                        logger.info("SQLite сессия успешно авторизована")
                        await test_client.disconnect()
                        return encoded
                    await test_client.disconnect()
                except Exception as e:
                    logger.error(f"Ошибка проверки SQLite сессии: {e}")
                
                return encoded
            
            # Пробуем как строку в разных кодировках
            for encoding in ['utf-8', 'latin-1', 'cp1251']:
                try:
                    text = content.decode(encoding).strip()
                    if text and len(text) > 50:
                        logger.info(f"Файл прочитан как текст ({encoding}), длина: {len(text)}")
                        return text
                except:
                    continue
...
async def process_account_file(message: Message, state: FSMContext):
    ...
    if message.document:
        ...
        try:
            ...
            logger.info(f"Файл сохранен во временный: {temp_path}")
    ...
    # Обработка текста (если прислали строку сессии текстом)
    elif message.text:
    ...
    if not session_string:
        await message.answer(
            ...
            '<i>Убедитесь, что файл является валидной сессией Telethon</i>\n'
            'Поддерживаются:\n'
            '• .session файлы\n'
            '• Текстовые строки сессии\n'
